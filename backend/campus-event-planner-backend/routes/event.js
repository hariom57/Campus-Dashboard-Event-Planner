...
// Public route to get all current events (no auth required for browsing)
    ...
    try {
        const events = await sql`
            SELECT 
                ...
                (SELECT STRING_AGG(ec.name, ', ')
                 FROM event_category_alloted eca
                 JOIN event_category ec ON eca.event_category_id = ec.id
                 WHERE eca.event_id = event.id) AS categories
            FROM event
...
// Public route to get particular event details by event ID
